...
function amountTextClass(type: 'income' | 'expense' | 'transfer'): string {
  const map = {
    income:   'text-[#1A7D4F]',
    expense:  'text-[#C0392B]',
...
export default function EditTransactionDialog({
  ...
  // Pre-populate when transaction changes
  useEffect(() => {
    if (transaction) {
      reset({
  ...
  // Auto-fill exchange_rate only if the existing transaction had none
  const watchedCurrency = watch('currency');
  useEffect(() => {
    if (watchedCurrency === 'SYP' && latestRate != null && !transaction?.exchange_rate) {
      setValue('exchange_rate', String(latestRate));
    } else if (watchedCurrency === 'USD') {
      setValue('exchange_rate', '');
    }
  }, [watchedCurrency, latestRate, setValue, transaction]);
      ...
      <DialogContent className="max-w-md">
        ...
        <form onSubmit={onSubmit} className="space-y-4 pt-2">
          ...
          <DialogFooter className="pt-2">
            ...
            >
              {isSubmitting
                ? t('transactions.dialog.updating')
                : t('transactions.dialog.updateSubmit')}
            </Button>
          </DialogFooter>
