...
const addPropertySchema = z.object({
  ...
  // HTML number inputs return strings — validate as string, coerce at submit
  estimated_value: z
    .string()
    .refine(
...
export function AddPropertyDialog({ open, onOpenChange }: AddPropertyDialogProps) {
  ...
  const onSubmit = async (data: AddPropertyFormData) => {
    ...
    if (error) {
      toast.error(t('properties.toast.addError'));
      return;
    }
      ...
      >
        <DialogHeader>
          <DialogTitle className="text-lg font-medium text-[#1E293B]">
            {t('properties.form.dialogTitle')}
          </DialogTitle>
          <DialogDescription className="sr-only">
            {t('properties.form.dialogDescription')}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            >
              {isSubmitting ? (
                ...
                t('properties.form.submitButton')
              )}
            </Button>
          </div>
